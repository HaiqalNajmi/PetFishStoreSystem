Public Class frm_makeorder_a174863
    ...
    Private Sub frm_makeorder_a174863_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        refresh_grid()
        get_current_code()
    ...
    Private Sub refresh_text(product As String)
        ...
        Try
            ...
            pic_product.BackgroundImage = Image.FromFile("pictures/" & lbl_productid2.Text & ".jpg")
    ...
    Private Sub refresh_grid()
        Dim mysql2 As String = "SELECT * FROM TBL_CUSTOMER_A174863"
    ...
    Private Sub refresh_staff()
        ...
        Dim mysql As String = "SELECT FLD_STAFF_ID FROM TBL_STAFF_A174863"
        Dim mydatatable As New DataTable
        Dim myreader As New OleDb.OleDbDataAdapter(mysql, myconnection)
        myreader.Fill(mydatatable)
    ...
    Private Sub refresh_count()
        ...
        Dim mysql As String = "SELECT COUNT (FLD_ORDER_ID) As count_id FROM TBL_ORDER_A174863"
        Dim mydatatable As New DataTable
        Dim myreader As New OleDb.OleDbDataAdapter(mysql, myconnection)
        myreader.Fill(mydatatable)
    ...
    Private Sub btn_neworder_Click(sender As System.Object, e As System.EventArgs) Handles btn_neworder.Click
        ...
        Try
            ...
            Dim mysql As String = "INSERT INTO TBL_ORDER_A174863 (FLD_ORDER_ID, FLD_CUSTOMER_ID, FLD_STAFF_ID, FLD_DATE) values (""" &
            lbl_orderid2.Text & """,""" & lbl_customerid2.Text & """,""" & cbb_staffid.Text & """,""" & txt_orderdate.Text & """)"
    ...
    Private Sub btn_remove_Click(sender As System.Object, e As System.EventArgs) Handles btn_remove.Click
        Dim delete_order = MsgBox("Confirm to delete this item from the list?", MsgBoxStyle.YesNo)
    ...
    Private Sub btn_confirm_Click(sender As System.Object, e As System.EventArgs) Handles btn_confirm.Click
        ...
        Try
            For i As Integer = 0 To grd_orderdetails.RowCount - 1
                ...
                Dim mysql As String = "insert into TBL_ITEM_LIST_A174863 (FLD_ORDER_ID, FLD_PRODUCT_ID, FLD_QUANTITY, FLD_PRICE) values (""" & lbl_orderid2.Text & """,""" & productid & """, """ & quantity & """, """ & price & """)"
                Dim mywriter As New OleDb.OleDbCommand(mysql, myconnection2, mytransaction)
